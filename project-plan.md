...
*(Wireframes below will be recreated as polished mockups in Figma / hand-drawn sketches for submission.)*
...
| ----- | ----- |
| **Jack** | **Backend/Routes Lead** — develops Express routes for full CRUD on games and sessions, implements the RAWG API integration server-side helpers, and connects routes to the MySQL database. |
| **Tim** | **Database Lead** — designs the schema and ERD, writes table creation and seed insert statements, exports the final `.sql` file, and codes at least two backend query functions (e.g., stats aggregation). |
| **Glenn** | **JavaScript/Frontend Lead** — writes the external client-side JS: validation, `fetch()` calls to RAWG and CheapShark, `localStorage` handling, and DOM filtering/sorting. Also codes at least one backend route (wishlist CRUD). |
| **Eric** | **UI/Integration Lead** — builds EJS views and partials, applies Bootstrap \+ custom CSS (50+ rules), ensures responsive design and consistent theming, and codes at least one backend route (play session logging). |
...
| user\_name | VARCHAR(50) | owner of the entry |
...
| cover\_url | VARCHAR(255) | from RAWG API |
...
| price\_limit | DECIMAL(6,2) | buy when deal is below this |
| saved\_at | DATETIME | default NOW() |
